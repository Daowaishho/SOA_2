...
//  序列化接口
        Serializable{
    @Query("SELECT r FROM Reserve r WHERE r.delFlag AND r.roomId = ?1 AND r.beginTime > ?2 AND r.beginTime < ?3 OR r.beginTime <= ?2 AND r.endTime >= ?3 OR r.endTime > ?2 AND r.endTime < ?3")
